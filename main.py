...
def data_bin_collect(class_file:str)->List[float]:
    ...
    return np.array(np.nan_to_num(histogram[1:]), dtype=float)
    

if __name__ == "__main__":
   
   mrna_bins:List[float] = data_bin_collect(mrna_file)
   ncrna_bins:List[float] = data_bin_collect(ncrna_file)

#    print(mrna_bins)
#    print(ncrna_bins)

   classification_model =  model(
       ncrna_bins=ncrna_bins,
       mrna_bins=mrna_bins)
